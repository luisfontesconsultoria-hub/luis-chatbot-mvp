...
const {importCompanies,normalizeCompany,validPhone}=require('./captacao-import');
...
async function run(){
  ...
  assert.equal(validPhone('51999998888'),true); assert.equal(validPhone('123'),false);
  ...
  let r=await importCompanies({repository:repo,source:'GOOGLE_MAPS',rows:[{name:'A',phone:'(51) 99999-0000',cnpj:'11.111.111/0001-11'}]});
  assert.equal(r.status,200); assert.equal(r.body.summary.created,1); assert.equal(rows[0].source,'GOOGLE_MAPS'); assert.ok(rows[0].raw_data);
  ...
  r=await importCompanies({repository:repo,rows:[{phone:'51999990004'},{name:'E'},{name:'F',phone:'123'}]});
  assert.equal(r.body.summary.rejected,3);
