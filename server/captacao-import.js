...
function validPhone(phone){return typeof phone==='string' && phone.length>=10 && phone.length<=13}

async function importCompanies({repository,rows=[],source='IMPORT'}={}){
  ...
  for(let i=0;i<rows.length;i++){
    ...
    if(!validPhone(payload.phone)){results.push({row:i+1,status:'rejected',error:'PHONE_INVALID'});continue}
    try{
      ...
      const company=await repository.createCapturedCompany({...payload,source:payload.source||source,raw_data:rows[i]});
...
module.exports={importCompanies,normalizeCompany,cleanDigits,validPhone};
