  ...
  for (const entry of (Array.isArray(payload.entry) ? payload.entry : [])) {
    for (const change of (entry.changes || [])) {
      for (const message of ((change.value || {}).messages || [])) {
        messages.push({
          channel:'WHATSAPP', external_message_id:message.id || null, phone:message.from || null,
          timestamp:message.timestamp ? new Date(Number(message.timestamp)*1000).toISOString() : new Date().toISOString(),
          type:message.type || 'other', text:message.text?.body || '', media_url:null, source:'WHATSAPP', campaign:null
        });
      }
    }
  }
...
async function routeRequest({ method, path, query={}, body={}, rawBody='', signatureHeader='' } = {}) {
  ...
  if (method==='POST' && path==='/webhooks/meta') {
    ...
    if (!config.metaAppSecret || !verifyMetaSignature(rawBody, signatureHeader, config.metaAppSecret)) return { status:401, body:{ error:'META_SIGNATURE_INVALID' } };
    const messages=normalizeMetaWebhook(body);
    if (!runtime.pipeline) return { status:503, body:{ error:'PERSISTENCE_NOT_CONFIGURED' } };
    if (messages.length) runtime.pipeline(messages).catch(error => console.error('WEBHOOK_PIPELINE_ERROR', error?.message || 'PIPELINE_ERROR'));
