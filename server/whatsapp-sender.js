...
function createWhatsAppSender({ accessToken = process.env.META_ACCESS_TOKEN, phoneNumberId = process.env.META_PHONE_NUMBER_ID, request = https.request } = {}) {
  return {
    async sendText({ to, text }) {
      if (!accessToken || !phoneNumberId) throw new Error('META_SEND_CREDENTIALS_REQUIRED');
      ...
      return new Promise((resolve,reject)=>{
        const req=request({ hostname:'graph.facebook.com', path:`/vXX.X/${phoneNumberId}/messages`, method:'POST', headers:{ Authorization:`Bearer ${accessToken}`, 'Content-Type':'application/json', 'Content-Length':Buffer.byteLength(body) } }, res=>{
