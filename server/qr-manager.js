...
function buildSpoolProcessor({repository,pipeline,now=()=>Date.now()}){
  const persistOnly=createWebhookPipeline({repository});
...
async function replaySpool(id){const proc=spoolProcessors.get(id);if(!proc)return null;const sp=getSpool(id);if(!sp.size())return null;const r=await sp.replay(proc);if(!r.skipped&&(r.done||r.dead))console.log('WHATSAPP_SPOOL_REPLAY',{slot:id,...r,pending:sp.size()});return r}
...
async function connect(slot,{repository,env=process.env,auto=false}={}){
  ...
  spoolProcessors.set(id,buildSpoolProcessor({repository,pipeline}));ensureSpoolTimer();
