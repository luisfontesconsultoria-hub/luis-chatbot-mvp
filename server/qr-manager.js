...
function status(slot){const id=slotId(slot),c=sessions.get(id);return{slot:id,status:c?.status||'DISCONNECTED',phone:c?.phone||null,qrDataUrl:c?.qrDataUrl||null,lastError:c?.lastError||null,experimental:true,provider:'BAILEYS_QR'}}
...
async function connect(slot,{repository,env=process.env}={}){
  ...
  if(old&&(old.status==='CONNECTING'||old.status==='QR_READY')){
    ...
    sessions.delete(id);clearAuthState(id);
  ...
  const current={status:'CONNECTING',qrDataUrl:null,phone:null,lastError:null,socket:null,reconnectTimer:null,qrTimer:null,startedAt:Date.now(),stopped:false,sentMessageIds:new Set()};
  ...
  sock.ev.on('messages.upsert',async({messages,type,requestId})=>{
    ...
    for(const msg of messages||[]){
      try{
        ...
        const outcomes=await pipeline([{channel:'WHATSAPP',external_message_id:externalId,phone:identity.phone,whatsapp_jid:identity.jid,name:msg.pushName||null,timestamp:messageTimestamp(msg.messageTimestamp),type:'text',text,media_url:null,source:`WHATSAPP_QR_SLOT_${id}`,campaign:null}]);
        const failure=(outcomes||[]).find(x=>x?.status==='error');
        if(failure){current.lastError=`MESSAGE_PIPELINE_ERROR:${failure.error||'PROCESSING_ERROR'}`;console.error('WHATSAPP_PIPELINE_ERROR',{slot:id,error:failure.error||'PROCESSING_ERROR'})}
        else{current.lastError=null;console.log('WHATSAPP_PIPELINE_OK',{slot:id,status:outcomes?.[0]?.status||'UNKNOWN'})}
      }catch(e){current.lastError=`MESSAGE_PIPELINE_ERROR: ${e.message}`;console.error('WHATSAPP_PIPELINE_EXCEPTION',{slot:id,error:e.message})}
    }
  });
  return status(id);
}
