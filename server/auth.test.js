...
assert(authenticate(session.token)?.user==='admin','session must authenticate');
logout(session.token);assert(!authenticate(session.token),'logout must invalidate session');
