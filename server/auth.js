...
function login(user,password,env=process.env){const c=credentials(env);if(!c.user||!c.password||user!==c.user||password!==c.password)throw new Error('INVALID_CREDENTIALS');const expiresAt=Date.now()+TTL_MS;return{token:makeToken(user,expiresAt,env),expiresIn:TTL_MS/1000,user}}
function authenticate(token,env=process.env){const raw=String(token||'');const parts=raw.split('.');if(parts.length!==4||parts[0]!=='v1')return null;const payload=parts.slice(0,3).join('.');const expected=sign(payload,env);const a=Buffer.from(parts[3]);const b=Buffer.from(expected);if(a.length!==b.length||!crypto.timingSafeEqual(a,b))return null;const expiresAt=Number(parts[2]);if(!Number.isFinite(expiresAt)||expiresAt<Date.now())return null;let user='';try{user=Buffer.from(parts[1],'base64url').toString('utf8')}catch{return null}const c=credentials(env);if(!user||user!==c.user)return null;return{user,expiresAt}}
function logout(_token){return true}
