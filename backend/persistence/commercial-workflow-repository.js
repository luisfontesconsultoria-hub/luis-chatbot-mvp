...
async function recordAppointmentOutcome(repo, leadId, appointment) {
  ...
  const updated = await repo.updateLead(leadId, { status: result.lead.status, stage: result.lead.stage, nextAction: result.nextAction });
...
async function recordVisitOutcome(repo, leadId, resultValue, metadata={}) {
  ...
  const updated = await repo.updateLead(leadId, { stage: result.lead.stage, nextAction: result.nextAction });
