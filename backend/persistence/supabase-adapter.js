...
function createSupabaseRepository(client) {
  ...
  return assertRepository({
    ...
    async createEvent(payload){const row={lead_id:payload.lead_id||null,type:payload.type||payload.event_type||'SYSTEM',idempotency_key:payload.idempotency_key||null,payload:payload.payload||payload.metadata||{}};const{data,error}=await client.from(TABLES.events).insert(row).select('*').single();if(error)throw error;return data},
