...
function visitResultToStatus(result){const v=String(result||'').toUpperCase();if(['CONVERTED','CONVERTIDO','FECHADO','GANHO'].includes(v))return'CONVERTIDO';if(['NO_INTEREST','SEM_INTERESSE','LOST','PERDIDO'].includes(v))return'PERDIDO';if(['FOLLOW_UP','RETORNO','AGUARDANDO_RETORNO'].includes(v))return'AGUARDANDO_RETORNO';return null}
